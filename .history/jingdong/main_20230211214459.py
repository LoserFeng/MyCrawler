...
def main():
    ...
    for i in range(0,TOTAL_PAGE):
        ...
        product_name_list.extend(re.findall(r'<a target="_blank" title="(.*?)" href=',page,re.S)) 
        product_shop_list.extend(re.findall(r'<a target="_blank" title="(.*?)" href=',page,re.S))   
        #product_id_list.extend(re.findall())


    print(product_name_list)
    print(product_price_list)
    print(f'product_id_list:{len(product_id_list)},product_price_list:{len(product_price_list)},product_name_list:{len(product_name_list)}')
